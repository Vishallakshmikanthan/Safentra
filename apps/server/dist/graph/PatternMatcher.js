...
class PatternMatcher {
    ...
    // Pattern 1: Gas Elevation + Hot Work
    pattern1_GasElevationHotWork(zone) {
        const sensors = this.graph.getSensorsByZone(zone.id);
        const gasSensors = sensors.filter(s => s.type === 'h2s_concentration' || s.type === 'gas_pressure' || s.type === 'co_concentration');
        ...
        const factors = [];
        if (hasGasElevation)
            factors.push('Gas sensor elevated/alarm');
        if (hasHotWork)
            factors.push('Active hot work permit');
        return {
            patternId: 'P1',
    ...
    // Pattern 2: Confined Space + Gas Elevation
    pattern2_ConfinedSpaceGasElevation(zone) {
        const sensors = this.graph.getSensorsByZone(zone.id);
        const gasSensors = sensors.filter(s => s.type === 'h2s_concentration' || s.type === 'co_concentration' || s.type === 'oxygen_level');
        const hasGasElevation = gasSensors.some(s => s.status === 'elevated' || s.status === 'alarm');
        ...
        const factors = [];
        if (hasGasElevation)
            factors.push('Toxic gas elevated in confined space');
        if (hasConfinedSpace)
            factors.push('Active confined space entry permit');
        return {
            patternId: 'P2',
    ...
    // Pattern 3: Shift Changeover + Active Permits
    pattern3_ShiftChangeoverActivePermits(zone) {
        ...
        const factors = [];
        if (isShiftChangeover)
            factors.push('Shift changeover active');
        if (hasActivePermits)
            factors.push(`${activePermits.length} active permit(s) during handover`);
        return {
            patternId: 'P3',
    ...
    // Pattern 4: Multi-Zone Gas Migration
    pattern4_MultiZoneGasMigration(zone) {
        const sensors = this.graph.getSensorsByZone(zone.id);
        const gasSensors = sensors.filter(s => s.type === 'h2s_concentration' || s.type === 'gas_pressure');
        ...
        for (const adjZone of adjacentZones) {
            ...
            const adjGas = adjSensors.filter(s => s.type === 'h2s_concentration' || s.type === 'gas_pressure');
        ...
        const factors = [];
        if (localElevated)
            factors.push('Local gas elevation detected');
        if (adjacentElevated > 0)
            factors.push(`${adjacentElevated} adjacent zone(s) with gas elevation`);
        return {
            patternId: 'P4',
    ...
    // Pattern 5: Worker Isolation + Gas Alarm
    pattern5_WorkerIsolationGasAlarm(zone) {
        ...
        const isolatedWorkers = activeWorkers.filter(w => w.status === 'in_confined_space' ||
            (zone.hazardClass === 'A' && activeWorkers.length <= 2));
        const sensors = this.graph.getSensorsByZone(zone.id);
        const hasGasAlarm = sensors.some(s => (s.type === 'h2s_concentration' || s.type === 'co_concentration' || s.type === 'oxygen_level')
            && s.status === 'alarm');
        const matched = isolatedWorkers.length > 0 && hasGasAlarm;
        const confidence = matched ? 0.95 : 0;
        const factors = [];
        if (isolatedWorkers.length > 0)
            factors.push(`${isolatedWorkers.length} isolated worker(s) in zone`);
        if (hasGasAlarm)
            factors.push('Gas alarm active');
        return {
            patternId: 'P5',
    ...
    // Pattern 6: Temperature Rise + Gas Pressure
    pattern6_TemperatureRiseGasPressure(zone) {
        const sensors = this.graph.getSensorsByZone(zone.id);
        ...
        const factors = [];
        if (tempRising)
            factors.push('Temperature rising above normal');
        if (pressureHigh)
            factors.push('Gas pressure elevated');
        return {
            patternId: 'P6',
    ...
    // Pattern 8: Sensor Drift + No Calibration
    pattern8_SensorDriftNoCalibration(zone) {
        const sensors = this.graph.getSensorsByZone(zone.id);
        // Simplified: check for sensors that have been elevated for extended period
        const staleElevated = sensors.filter(s => s.status === 'elevated' &&
        ...
        const factors = [];
        if (matched)
            factors.push(`${staleElevated.length} sensor(s) elevated for >1hr without resolution`);
        return {
            patternId: 'P8',
    ...
    // Pattern 9: Conveyor Proximity + Worker Presence
    pattern9_ConveyorProximityWorker(zone) {
        ...
        if (proximityTriggered)
            factors.push('Proximity sensor triggered');
        if (workersPresent)
            factors.push(`${activeWorkers.length} worker(s) near conveyor`);
        return {
            patternId: 'P9',
    ...
    // Pattern 10: Oxygen Depletion + Confined Space
    pattern10_OxygenDepletionConfinedSpace(zone) {
        const sensors = this.graph.getSensorsByZone(zone.id);
        ...
        const factors = [];
        if (oxygenLow)
            factors.push(`Oxygen level at ${oxygenSensor?.currentValue.toFixed(1)}%`);
        if (hasConfinedSpace)
            factors.push('Confined space entry active');
        return {
            patternId: 'P10',
    ...
    // Pattern 11: Cumulative Safety Debt Threshold
    pattern11_CumulativeSafetyDebt(zone) {
        const debtThreshold = 70;
        const matched = zone.safetyDebt >= debtThreshold;
        const confidence = matched ? 0.8 : 0;
        const factors = [];
        if (matched)
            factors.push(`Safety debt at ${zone.safetyDebt} (threshold: ${debtThreshold})`);
        return {
            patternId: 'P11',
    ...
    pattern12_VizagFourCondition(zone) {
        const sensors = this.graph.getSensorsByZone(zone.id);
        const gasSensors = sensors.filter(s => s.type === 'h2s_concentration' || s.type === 'gas_pressure' || s.type === 'co_concentration');
        const hasGasElevation = gasSensors.some(s => s.status === 'elevated' || s.status === 'alarm');
        const isShiftChangeover = this.graph.isShiftChangeover();
    ...
    // Pattern 13: Compound Risk Escalation (Risk Score Acceleration)
    pattern13_CompoundRiskEscalation(zone) {
    ...
    createEmptyResult(patternId, patternName, zoneId) {
        return {
            patternId,
            patternName,
            zoneId,
