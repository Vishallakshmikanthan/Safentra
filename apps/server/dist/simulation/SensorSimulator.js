 ...
 */
class SensorSimulator extends events_1.EventEmitter {
    graph;
    profiles;
    ...
    dangerActivatedAt = 0;
    recoveryStartedAt = 0;
    recoveryStartValues = new Map();
    dangerEventsEmitted = new Set();
    tickInterval = null;
    driftInterval = null;
    TICK_MS = 2000;
    constructor(graph) {
     ...
     */
    computeDangerValue(profile, elapsedSeconds) {
        const tau = 15;
     ...
     */
    computeRecoveryValue(profile, startValue, elapsedSinceRecovery) {
        const tau = 10;
    ...
    tick() {
        ...
        const correlationOffsets = new Map();
        for (const p of this.profiles) {
    ...
    getDangerElapsedSeconds() {
        if (!this.dangerMode || this.dangerActivatedAt === 0)
            return 0;
