...
export class RiskScorer {
  private emaAlpha = 0.3; // EMA smoothing factor
  private confirmationWindow = 3; // 3-tick confirmation window
  private confirmationTicks: Map<string, number> = new Map();
  private previousRawScores: Map<string, number> = new Map();

  calculateRiskScore(zone: Zone, patternResults: PatternResult[]): { riskScore: number; rawRiskScore: number; severity: RiskSeverity } {
    ...
    if (finalScore >= 0.8) severity = 'critical';
    else if (finalScore >= 0.6) severity = 'high';
    else if (finalScore >= 0.3) severity = 'medium';
  ...
  private getSeverityFromScore(score: number): RiskSeverity {
  ...
  private getRecommendedAction(patternId: string, zone: Zone): string {
  ...
  getConfirmationStatus(zoneId: string): { ticks: number; required: number; confirmed: boolean } {
